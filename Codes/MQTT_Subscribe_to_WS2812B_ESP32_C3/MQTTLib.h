...
void TurnOnBulb();  // 開啟燈泡全亮(白光全亮設定)
void TurnOffBulb(); // 關閉燈泡全暗(無亮度設定全暗)
...
//  Serial.print("\n"); //印出換行鍵
  ...
  {
    if (json_doc["Style"] == "COLOR") //如果json之"Style"元素內容為"COLOR":控制彩色
    {
       int r = json_doc["Color"]["R"] ; //取得紅色256階層值
       int g = json_doc["Color"]["G"] ; //取得綠色256階層值
       int b = json_doc["Color"]["B"] ; //取得藍色256階層值
      ChangeBulbColor(r,g,b) ;  // 改變燈泡顏色函數 
      Serial.println("改變燈泡顏色函數") ;   //印出改變燈泡顏色函數
    }
    else
    {
       if (json_doc["Style"] == "MONO") //如果json之"Style"元素內容為"MONO":代表只管明滅，白光或熄滅
        {
         if (json_doc["Command"] == "ON") //如果json之"Command"元素內容為"ON":開啟白光燈泡
          {
            TurnOnBulb();// 開啟燈泡全亮(白光全亮設定)
            Serial.println("開啟燈泡全亮") ;   //印出開啟燈泡全亮
          }
          else if (json_doc["Command"] == "OFF") //如果json之"Command"元素內容為"OFF":關閉白光燈泡
              {
                TurnOffBulb();  // 關閉燈泡全暗(無亮度設定全暗)
                Serial.println("關閉燈泡全暗") ;   //印出關閉燈泡全暗
              }
        }
        else
        {
